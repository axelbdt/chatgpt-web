<script lang="ts">
  ...
  const showChatNameSettings = () => {
    chatNameModalActive = true;
  };

  const saveChatNameSettings = (event) => {
    const newChatName = event.detail;
    // save if changed
...
<ChatRenameModal
  ...
  on:save={saveChatNameSettings}
  initialChatName={chat.name || `Chat ${chat.id}`}
/>
<!-- end -->
